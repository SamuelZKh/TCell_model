...
def time_evolve(nx,ny,c,p,psi,V0):
    ...
    for i in range(num_points_x):
        for j in range(num_points_y):
            ...
            dc[i,j] = dt *(psi[i,j]*Da*laplacian_c-psi[i,j]*v0*(grad_cnx+grad_cny)-psi[i,j]*W*(c[i,j]*laplacian_p+grad_px*grad_cx+grad_cy*grad_py))
            dp[i,j]=dt*(psi[i,j]*Dm*laplacian_p-psi[i,j]*v0*(grad_pnx+grad_pny)+psi[i,j]*kb*(c[i,j]/(c[i,j]+ch))-psi[i,j]*ku*(p[i,j]))
            dnx[i,j]=dt*(-lam*(nx[i,j]*grad_nx_x+ny[i,j]*grad_nx_y)+K*laplacian_nx-eta0*grad_cx*psi[i,j]+eta*grad_px*psi[i,j]+(alpha-beta*(nx[i,j]**2+ny[i,j]**2))*nx[i,j]*psi[i,j])
            dny[i,j] = dt * (-lam * (ny[i,j] * grad_ny_x + nx[i,j] * grad_ny_y) + K* laplacian_ny - eta0 * grad_cy*psi[i,j]  + eta * grad_py*psi[i,j] + (alpha - beta * (nx[i,j]**2+ny[i,j]**2)) * ny[i,j]*psi[i,j])
...
c = (C*(np.ones((num_points_x,num_points_x))+ Csig*np.random.rand(num_points_x,num_points_x)))*psi

p = P*(np.ones((num_points_x,num_points_x))+ Psig*np.random.rand(num_points_x,num_points_x))*psi
