...
max_polymer_radius = 200  # Set the maximum radius for polymerization to stop
...
for t in range(sim_time):
    ...
    for i, branch in enumerate(actin_store):
        ...
        for k, mono in enumerate(actin_pool_positions):
            # print(np.sqrt((mono[0]-branch[-1][0])**2+(mono[1]-branch[-1][1])))

            if len(branch) > 0 and np.sqrt((mono[0] - branch[-1][0]) ** 2 + (mono[1] - branch[-1][1])) <= polymerise_region:
        ...
        if polymerise < polymerise_prob and len(available_monomer) > 0 and len(actin_pool_positions)> 0 :
            ...
            if len(actin_store[i]) > 0:
                ...
                for k, mono in enumerate(actin_pool_now):
                    ...
                    if len(branch) > 0 and np.sqrt(
                            (x - branch[-1][0]) ** 2 + (y- branch[-1][1])) <= polymerise_region:
                        available_monomer.append(mono)
                        ind_available_monomer.append(k)
                        break
