...
for t in range(sim_time):
    ...
    for j, particle in enumerate(particle_store):
        ...
        for k, branch in enumerate(actin_store):
            ...
            if (particle_in_time[j]) not in branch and particle_bound_status[j]==1:
                print('diss')
...
def update_frame(frame):
    ...
    act_pool = actin_pool_time[frame]

    xa = [p[1] for p in act_pool]
    ya = [p[0] for p in act_pool]
    plt.scatter(xa, ya, color='blue', s=3)
