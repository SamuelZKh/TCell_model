...
max_polymer_radius = 300  # Set the maximum radius for polymerization to stop
...
for t in range(sim_time):
    ...
    actin_last_time=[]
    actin_first_time=[]
    actin_pool_now=actin_pool_positions

    for k,mono in enumerate(actin_pool_now):
        mono[0] += np.random.normal(0, 0.5)
        mono[1] += np.random.normal(0, 0.5)

        actin_pool_positions[k] = [mono[0], mono[1]]


    for i, branch in enumerate(actin_store):   ## implementign periodic growing and decayin (like tide)
        polymerise_prob = 0.1
        depolymeris_prob = 0.5
        if t in range(0,50) and branch[0][1]>175:

            polymerise_prob =0.5
        if t in range(50,100) and branch[0][1]<175:
       ...
       # print(polymerise_prob)
        ...
        if len(branch) > 0:
            ...
            if len(branch) <= 20:
                ...
                if polymerise < polymerise_prob  and len(actin_pool_now) > 0:
        ...
        for k, mono in enumerate(actin_pool_now):
            #print(np.sqrt((mono[0]-branch[-1][0])**2+(mono[1]-branch[-1][1])))
            if np.sqrt((mono[0]-branch[-1][0])**2+(mono[1]-branch[-1][1]))<=2:
                available_monomer.append(mono)
                ind_available_monomer.append(k)
                break

        #print('asdadsasd',available_monomer)
        if polymerise < polymerise_prob  and tot_actin > 0 :
            #actin_pool_positions.pop(ind_available_monomer[0])
        ...
        if depolymerise < depolymeris_prob :
            ...
            if depolymerise < shortening and t in range(50,100) and branch[0][1]>175:
                if len(actin_store[i]) > 100:

                    actin_store[i] = actin_store[i][:-polymer_size]
                    tot_actin += 1
            if depolymerise < shortening and t in range(100,150) and branch[0][1]<175:
                if len(actin_store[i]) > 100:

                    actin_store[i] = actin_store[i][:-polymer_size]
                    tot_actin += 1

    actin_pool_time.append(list(actin_pool_positions))
    ...
    for j, particle in enumerate(particle_store):
        ...
        for k, branch in enumerate(actin_store):
            for m in range(len(branch) - 1):
                ...
                if np.sqrt((x - monomer_x) ** 2 + (y - monomer_y) ** 2) <= particle_radius and (
                        particle_bound_status[j] == 0 or particle_bound_status[j] == 2) :
        ...
        if (particle_in_time[j]) not in actin_store[branch_bound[j]] and (
                particle_bound_status[j] == 1 or particle_bound_status[j] == 3) and branch_bound[
            j] != 0:
            # print('diss')
