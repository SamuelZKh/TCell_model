...
def time_evolve(nx,ny,c,p,psi,V0):
    ...
    for i in range(num_points_x):
        for j in range(num_points_y):
            ...
            dc[i,j] = dt *(Da*laplacian_c-v0*(grad_cnx+grad_cny)+W*(c[i,j]*laplacian_p+grad_cx*grad_px+grad_cy*grad_py))*psi[i,j]
...
for i in range(num_points_x):
    for j in range(num_points_y):
        if (i - center_x) ** 2 + (j - center_y) ** 2 <= (radius/1.5) ** 2:
            Vc_i[i, j] = 1
...
for t in range(num_steps):
    ...
    if t%50000==0:
        ...
        print('sun:C',np.sum(c)+np.sqrt(np.sum(nx)**2+np.sum(ny)**2))
        ...
        plt.quiver(X, Y, nx*c, ny*c, alpha=0.5)
        ...
        with open('P_field.txt', 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerows(p)
        # Save the T matrix values to the CSV file
        ...
        with open('psi_field.txt', 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
