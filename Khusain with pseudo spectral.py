...
c_k = np.empty((num_points_x,num_points_y), dtype=np.complex64)
p_k = np.empty((num_points_x,num_points_y), dtype=np.complex64)



kx = np.fft.fftfreq(num_points_x, d=dx)*2*np.pi
...
def c_Noperator_func(c,p):
    return W*(k[0]*k[0]*fft2((c*p))+k[1]*k[1]*fft2(c*p))





c_linear_k = np.exp(dt*c_Loperator_k )
c_non_k = dealias*dt*c_linear_k
