...
def time_evolve(nx,ny,c,p):
    ...
    for i in range(num_points_x):
        for j in range(num_points_y):
            ...
            dnx[i,j]=dt*(-lam*(nx[i,j]*grad_nx_x+ny[i,j]*grad_nx_y)+K*laplacian_nx+eta0*grad_cx+alpha[i,j]*nx[i,j]+eta*grad_px+(alpha[i,j]-beta[i,j]*n_sq[i,j])*nx[i,j])
            dny[i,j] = dt * (-lam * (ny[i,j] * grad_ny_x + nx[i,j] * grad_ny_y) + K* laplacian_ny + eta0 * grad_cy + alpha[i,j] * ny[i,j] + eta * grad_py + (alpha[i,j] - beta[i,j] * n_sq[i,j]) * ny[i,j])
...
for t in range(num_steps):
    ...
    if t%5000==0:
        ...
        plt.imshow(p, cmap='coolwarm', extent=[0, length_x, 0, length_y], origin='lower', aspect='auto')
        plt.xlabel('x')
        plt.ylabel('y')
